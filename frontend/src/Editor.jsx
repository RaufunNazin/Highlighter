...
const Editor = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await api.post("/create_segments/", formData, {
        headers: {
      ...
      localStorage.setItem("lastVideo", video_url);
  ...
  return (
    <div className="space-y-12">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-8 text-center glass-panel bg-white/80">
          ...
          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-slate-900">Neural Processing...</h2>
            <p className="text-slate-500 italic">"Analyzing semantics with precision layers..."</p>
          </div>
        </div>
      ) : (
